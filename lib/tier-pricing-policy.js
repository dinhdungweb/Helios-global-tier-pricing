  ...
  return {
    ...
    gift: {
      ...
      minimums: normalizeMinimums(
        parseJson(env.FREE_GIFT_MINIMUMS_JSON, { USD: 0 })
      )
    }
  };
}
...
function getCustomerTotalSpent(customer) {
  const metafieldValue = Number(
    customer.totalSpentMetafield && customer.totalSpentMetafield.value
  );
  ...
  const amountSpent = Number(
    customer.amountSpent && customer.amountSpent.amount
  );
